...
const numbertwo = document.querySelector('#userInputLocation')
const btnSubmit = document.querySelector('#locationConfirm')
const btnReset = document.querySelector('#reset')

// const userValue = numbertwo.value
  ...
  async function getLocation() {
    try {
      ...
      musicResults.forEach(result => {
        ...
        `
        document.querySelector('#btn-music').insertAdjacentHTML('afterbegin',musicList)
      })
      ...
      sportResults.forEach(result => {
        ...
        <li class="list-search">${result.dates.start.localDate}</li>
        `
        document.querySelector('#btn-sport').insertAdjacentHTML('afterbegin',sportList)
      })
      ...
      miscResults.forEach(result => {
        ...
        <li class="list-search">${result.dates.start.localDate}</li>
        `
        document.querySelector('#btn-misc').insertAdjacentHTML('afterbegin',miscList)
      })
      ...
      individualResults.forEach(result => {
        ...
        <li class="list-search">${result.dates.start.localDate}</li>
        `
        document.querySelector('#btn-individual').insertAdjacentHTML('afterbegin',individualList)
      })
  ...
  async function displayMoreInfo() {
    try {
      ...
      attachResults.forEach(result => {
        ...
        <li class="list-result" id="eventDate">Event Date: ${result.dates.start.localDate}</li>
        <li class="list-result" id="eventTime">Event Time is ${(result.dates.start.timeTBA === false ? result.dates.start.localTime : "TBA")}</li>
        <a href="${result.seatmap.staticUrl}">Venue Seatmap URL</a>
        </ul>
        `
        document.querySelector('.result-container').insertAdjacentHTML('afterbegin',appendToResultGrid)
      })
    } catch (error) {
      console.error(error)
    }
